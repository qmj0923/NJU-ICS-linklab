...
再来看.data表项，它Offset为0x7，所以它是在相对于.text节偏移量为0x7的地方被引用的。我们来看phase1.s中偏移量为0x7（相对.text节）的字节所在的指令：
...
phase2.o里面除了do_phase函数外，还有几个有着奇怪函数名的函数。因为这个阶段只能修改.text节，所以我们只关注有call puts的函数（否则的话我们可以修改重定位信息，使得我们可以在任意想要调用puts的位置进行调用。不过这样很麻烦，因为添加重定位表项会改变重定位表的大小，然后还要调整节头表内容等等一系列操作，几乎是在重构phase2.o）。
...
这个阶段需要我们填写被人为清零的重定位表项。

关于如何阅读重定位表项这个问题，我们在phase1中粗略地描述了一下。现在我们来结合phase5的文件详细描述一下。
...
这些表的内容这么多，我们该从哪里入手呢？不急，我们从重定位表开始一点一点研究。这个重定位表有好几个节，我们先来看.rel.text节的第一个表项。这个表项中的符号名（Sym. Name）为yAnKQn，该符号在其所在的节中的偏移量（Sym.Value）为0x20：
...
我们继续看符号表中的这个表项。我们发现这个符号的大小为52（0x34），Ndx为6。Ndx的值表示这个符号属于节头表中的第Ndx个表项。我们再来看节头表：
...
分析完一个表项，我们就大致掌握了分析重定位表的方法了。接下来要填写重定位表项的工作简直易如反掌。

#### part1

我们先来填前三个被清零的重定位表项。这三个表项被重定位的位置都位于transform_code函数中。题目pdf中已经给出了这个函数的代码框架。我们对应自己的程序修改了对应变量的名称，这样看起来更清楚一些：
...
根据重定位表我们知道，switch跳转表的重定位信息没有被清空（.rel.text节的第二个表项），所以我们盲猜这个函数中其余的重定位内容都和yAnKQn有关。我们在这个函数中找到了三个看起来需要被重定位的指令，它们与重定位表中第一个表项对应的指令一模一样：
...
这样我们就知道重定位表的前三个空表项本来是什么内容了：Offset列分别为0x9a、0xab、0xdd，其余列和.rel.text节的第一个表项一模一样。接下来`hexedit phase5.o`，.rel.text节位于phase5.o偏移量为0x7f8处（从节头表的第三个表项或者重定位表的第一行都可以得知这个信息）。.rel.text节的前三个表项（未被清零）在phase5.o中的存在形式是这样的：
...
我们发现这里面只包含了每个表项的Offset和Info，所以我们照猫画虎，把被清零表项的Offset和Info还原一下：
...
下面一个被清零的重定位表项稍微复杂一点，它要求我们还原一个调用函数的重定位表项。复杂之处在于，填这个表项没有例子可以抄，需要我们自己去定位。
...
接下来是encode_1函数。因为它和encode_2行为类似，所以我们直接对着encode_2函数的重定位表项填encode_1函数缺失的表项即可。我们抄这一行：
...
照抄该行的Info。然后在phase5.s中找到该缺失表项的Offset，填到被清零的这一行：
...
> ->在重定位表中一一对应Offset位于encode_1函数中的表项和Offset位于encode_2函数中的表项
>
> ->根据encode_1函数中缺失的重定位表项，找到对应的encode_2函数中的重定位表项，抄下Info
>
> ->根据对应的encode_2函数中的重定位表项，到phase5.s的encode_2函数中定位该指令
>
...
很明显第6个空表项要我们把符号encoder填进去。第7个表项只要照抄它上面那个表项就行了。我们不再赘述，直接展示修改结果：
...
call指令的下一条指令中又有一个重定位信息，它的类型是R_386_GOTPC。这个类型书上没有进行解释，我们只需要知道它的功能即可：add之前，%eax的值是当前的指令地址（我们可以用多种方式称呼它：当前%eip的值、刚刚那条call指令下一条指令的地址、该add指令相对于.text节的偏移量）；add之后，%eax的值是**全局偏移量表GOT**的地址。

再下一条指令仍然有重定位信息，这次重定位的符号我们很熟悉：.data。它重定位到偏移量0xf的地方。也就是说，`movl $0x2,0x0(%eax)`中的0x0会被重定位成.data相关的信息。这次的重定位类型是R_386_GOTOFF，它会计算符号的值与GOT地址之间的差值，在这里符号的值就是.data的地址。我们仍然只需要知道它的功能即可：%eax（现在%eax的值是GOT的地址）加上这个重定位之后的值，得到的值是addr(.data)+0x0，这个值是个地址，mov指令会把立即数0x2赋值到这个地址上。
...
对于局部符号（local symbol，也可称为本地符号，在这里即符号aaa和bbb），我们注意到这里并没有使用GOT的表项。代码只是利用了GOT的地址找到了.data节的地址，从而定位到了需要被赋值的局部符号。
...
观看phase6.s我们知道（不看也能猜到），get_pc_thunk函数的重定位表项的后面一条肯定是GOT的重定位表项。所以我们对照着已有表项，直接就能填掉三个空表项：
